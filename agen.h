...
public:
    ...
    Agen<T>& operator =(const Agen<T>& a); // asignación de árboles 
    ~Agen(); // destructor
private:
    struct celda {
        T elto;
...
inline void Agen<T>::eliminarRaiz()
{
    ...
    assert(r->hizq == NODO_NULO); // La raíz es hoja.
    delete(r);
    r = NODO_NULO;
}
...
typename Agen<T>::nodo Agen<T>::copiar(Agen<T>::nodo n) 
{
    ...
    if (n != NODO_NULO) 
    {
        ...
        { 
            m->hizq = copiar(n->hizq); // Copiar primer subárbol. 
            m->hizq->padre = m;
            // Copiar el resto de subárboles.
            nodo hijo = m->hizq; // Último subárbol copiado. 
            nodo hedrcho = n->hizq->heder; // Siguiente subárbol a copiar 
            while (hedrcho != NODO_NULO) 
            {
